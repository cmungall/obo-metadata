...
class HasProvenance(AnnotationPropertyMixin):
    ...
    def __post_init__(self, *_: List[str], **kwargs: Dict[str, Any]):
        ...
        self.contributor = [v if isinstance(v, Thing) else Thing(**as_dict(v)) for v in self.contributor]
...
slots.contributor = Slot(uri=DCTERMS.contributor, name="contributor", curie=DCTERMS.curie('contributor'),
                   model_uri=OMOSCHEMA.contributor, domain=None, range=Optional[Union[Union[dict, Thing], List[Union[dict, Thing]]]])
